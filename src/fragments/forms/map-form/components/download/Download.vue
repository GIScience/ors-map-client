<template>
  <div class="download-container" ref="downloadContainer">
    ...
    <v-btn v-else-if="jobData || vehicleData" class="edit-btn" flat style="background:white; margin-top:0" :title="$t('download.download')" @click="openDownload()"><v-icon color="primary">cloud_download</v-icon></v-btn>
    <v-dialog v-model="isDownloadModalOpen" max-width="600" attach="body" :persistent="true">
      <box customClass="download-modal" v-model="isDownloadModalOpen" background="white" closable @closed="closeDownload()">
        ...
        <h3 v-else-if="jobData" slot="header">
          {{$t('download.downloadJobs')}}
        ...
        <h3 v-else-if="vehicleData" slot="header">
          {{$t('download.downloadVehicles')}}
          <v-btn class="edit-btn" :style="{background: 'white'}" flat @click="copyToClipboard" :title="$t('download.copyToClipboard')">
            <v-icon color="primary">content_copy</v-icon>
          </v-btn>
        </h3>
