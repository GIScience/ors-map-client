<template>
  <div class="printing-template directions-template">
    ...
    <template v-for="(segment, segmentIndex) in activeRoute.properties.segments" :key="segmentIndex + 'header' ">
      <h3> {{$t('print.segment')}} {{Number(segmentIndex) + 1}} </h3>
      ...
      <template v-for="(step, stepIndex) in segment.steps" :key="segmentIndex + '_' + stepIndex">
        <div>
