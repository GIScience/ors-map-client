<template>
  <div class="option-container">
    ...
    <v-menu v-if="profile.vehicleTypes" class="profile-option-menu" v-model="subProfileIsOpen"
      transition="slide-y-transition" :close-on-click="true" :close-on-content-click="true" :open-on-hover="true" bottom>
      ...
      <v-list light style="background:white">
        <template v-for="(vehicleType, index) in profile.vehicleTypes">
          <v-list-tile :key="index" @click.stop="profileSelected(profile.slug, vehicleType)">
            <v-list-tile-title>
    ...
    <v-menu v-else-if="profile.nestedProfiles" class="profile-option-menu" v-model="subProfileIsOpen"
      ...
      <v-list light style="background:white">
        <template v-for="(nestedProfile, index) in profile.nestedProfiles">
          <v-list-tile :key="index" @click.stop="profileSelected(nestedProfile)">
            <v-list-tile-title>
              {{ getProfileTitle(nestedProfile)}}
              <v-icon v-if="nestedProfile === activeVehicleType" color="primary" small large>check</v-icon>
            </v-list-tile-title>
          </v-list-tile>
        </template>
      </v-list>
