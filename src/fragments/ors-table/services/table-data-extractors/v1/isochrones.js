...
class IsochronesBuilder {
  ...
  buildTableData = () => {
    ...
    return new Promise((resolve) => {
      ...
      context.responseData.features.forEach((feature) => {
        const value = context.humanizedValue(feature.properties.value)
  ...
  humanizedValue = (value) => {
    ...
    const distanceUnit = this.responseData.info.query.units || this.translations.meters
    // if is distance, use the distance unit, if is `time`, the value will always be in seconds
    const unit = this.responseData.info.query.range_type === 'distance' ? distanceUnit : this.translations.sec
    return `${value} ${unit}`
  }
}
